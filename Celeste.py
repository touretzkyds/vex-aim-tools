...
class Celeste(StateMachineProgram):
    ...
    class CmdScript(StateNode):
        def start(self,event):
            ...
            try:
                ...
                self.post_failure()
                return
            self.robot.openai.note_for_later(contents)
    ...
    class CmdFailed(AskGPT):
        def __init__(self, query_template, filler_fn=lambda x: ""):
            super().__init__()
    ...
    class CmdSearch(StateNode, LooseSpec):
        ...
        def setup(self):
            ...
            # Code generated by genfsm on Wed Aug 12 04:29:30 2026:
    ...
    def setup(self):
        ...
        datatrans8 = DataTrans(re.compile('#script ')) .set_name("datatrans8")
        datatrans8 .add_sources(dispatch) .add_destinations(script)
        ...
        datatrans14 = DataTrans(re.compile('#search ')) .set_name("datatrans14")
        datatrans14 .add_sources(dispatch) .add_destinations(search)
        
        datatrans15 = DataTrans(re.compile('#pilottoobject ')) .set_name("datatrans15")
        datatrans15 .add_sources(dispatch) .add_destinations(pilottoobject)
        
        datatrans16 = DataTrans(re.compile('#doorpass ')) .set_name("datatrans16")
        datatrans16 .add_sources(dispatch) .add_destinations(doorpass)
        
        datatrans17 = DataTrans(re.compile('#pickup ')) .set_name("datatrans17")
        datatrans17 .add_sources(dispatch) .add_destinations(pickup)
        ...
        completiontrans18 .add_sources(script) .add_destinations(say2)
        ...
        failuretrans9 .add_sources(turntoward) .add_destinations(statenode1)
        ...
        failuretrans12 .add_sources(pickup) .add_destinations(statenode2)
